...
import {
  ...
  TrendingUp, Plus, Search, MapPin, DollarSign
} from 'lucide-react'
...
export default async function CustomerDashboardPage() {
  // Fetch real count and recent shipments from db
  let shipments: any[] = []
  ...
  try {
    shipments = await db.shipment.findMany({
      ...
      include: { trackingEvents: { take: 1, orderBy: { timestamp: 'desc' } } },
    })

    totalShipments = await db.shipment.count()
    activeShipments = await db.shipment.count({
      where: {
        status: { in: ['LABEL_CREATED', 'PICKUP_SCHEDULED', 'PICKED_UP', 'IN_TRANSIT', 'OUT_FOR_DELIVERY'] },
      },
    })
    deliveredShipments = await db.shipment.count({
    ...
    totalSpent = Number(paidSum._sum.amount || 0)
  } catch (e) {
    // Falls back cleanly if database not yet migrated
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Overview of active consignments, live telemetry, invoices, and shipping records.
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {shipments.length === 0 ? (
            <div className="p-12 text-center text-slate-400 space-y-3">
              ...
              <p className="text-sm">No shipments created yet. Start by creating your first consignment.</p>
              <Button asChild size="sm" className="bg-[#6B2737] hover:bg-[#E85A24] text-white">
                <Link href="/shipping">Create Shipment</Link>
              </Button>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {shipments.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-4 font-medium text-slate-800">{s.recipientName}</td>
                      <td className="p-4">{s.recipientCity}, {s.recipientCountry}</td>
                      <td className="p-4 font-medium">{s.serviceType}</td>
                      <td className="p-4">
                        <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-orange-100 text-orange-800">
                          {s.status.replace(/_/g, ' ')}
                        </span>
                      </td>
                      <td className="p-4 font-bold text-slate-800">{formatCurrency(Number(s.totalAmount))}</td>
